...
public:
	bool iniCheck(int xpos, int ypos) {
		// 도화지(0,0 ~ 100, 100)
...
public:
	...
	void showCanvas() {
		cout << "좌상[" << upLeft.getX() << ", " << upLeft.getY() << "]" << endl;
		cout << "우하[" << downRight.getX() << ", " << downRight.getY() << "]" << endl;
	}
};

int main() {
	Point pos1;
	if (!pos1.iniCheck(-2, 4)) {
		cout << "초기화 실패" << endl;
	}
	if (!pos1.iniCheck(2, 4)) {
		cout << "초기화 실패" << endl;
