/*
	...
	이용하여 사용자가 구매한 로또 복권의 달성여부를 출력하는 프로그램
...
public:
	...
	LottoGenerator(int count) {   // 당첨번호와 보너스번호를 추출하는 
		makeRandom(count);
	}

	// 랜덤으로 로또 번호를 생성하여 set에 저장 후 리턴
...
// 응모번호와 당첨번호를 비교하여 일치하는 개수에 따른 순위수출
void counts(vector<UserLotto> user, LottoGenerator lotto) {
	...
	for (int i = 0; i < user.size(); i++) {
		...
		// 당첨번호와 사용자가 번호를 비교하는 과정 - 당첨번호에서 - 당첨번호에서 보너스 제외
